...
//      shape and the expected unit_amount (priceId equivalent — Pro = 39900¢)
...
describe("POST /api/checkout — Stripe integration", () => {
  it("calls stripe.checkout.sessions.create with the Pro price (39900¢) and proper success/cancel URLs", async () => {
    setupMocks();
    const { POST } = await loadRoute();
    ...
    // Pro plan price flows via line_items[0].price_data.unit_amount (39900¢).
